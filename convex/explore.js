...
export const getFeaturedEvents = query({
    ...
    handler: async (ctx, args) => {
        ...
        const events = await ctx.db.query("events").withIndex("by_start_date").filter((q) => q.gte(q.field("startDate"), now)).order("desc").collect()
...
export const getEventsByLocation = query({
    ...
    handler: async (ctx, args) => {
        ...
        let events = await ctx.db.query("events").withIndex("by_start_date").filter((q) => q.gte(q.field("startDate"), now)).order("desc").collect()
...
export const getEventsByCategory = query({
    ...
    handler: async(ctx, args) => {
        ...
        const events = await ctx.db.query("events").withIndex("by_category", (q) => q.eq("category", args.category))
...
export const getCategoryCounts = query({
    ...
    handler: async(ctx, args) => {
        ...
        const now = Date.now()
        const events = await ctx.db.query("events").withIndex("by_start_date").filter((q) => q.gte(q.field("startDate"), now)).collect();
